(ns regulation.actor
  "RegulationActor — the ISIC Rev.5 8413 economic regulation and business
  compliance administration actor as a `langgraph.graph/state-graph`
  (per ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  compliance or regulatory operation request (intake → advise → govern →
  ...
  The unconditional invariant: the RegulationAdvisor can never
  directly commit a record or dispatch a robot action the
  RegulationGovernor refuses — every commit-record! call is gated
  behind `:decide`. Additionally, any proposal touching permit grant/denial,
  binding tariff/rate/ruling setting, enforcement actions, or registration
  manipulation PERMANENTLY HOLDS (no path to approval)."
...
(defn build-graph
  "Build a compiled RegulationActor graph. `store` implements
  `regulation.store/Store`. `advisor` implements `regulation.advisor/Advisor`
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:business-id (:business-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
...
(defn run-request!
  "Run one compliance/regulatory operation request to completion or
